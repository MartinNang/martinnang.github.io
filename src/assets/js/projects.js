...
$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#sort-by-name-desc").click(() =>
        sortProjects(SortProperty.NAME, true)
      );
      $("#sort-by-date").click(() => sortProjects(SortProperty.DATE));
      $("#sort-by-date-desc").click(() =>
        sortProjects(SortProperty.DATE, true)
      );
    },
  });

  // TODO: fetch games from itch.io
  // $.ajax({
    ...
    switch (prop) {
      ...
      case SortProperty.NAME:
        if (sortBackwards) {
          ...
          console.log(
            "name: ",
            currentProjects[0]["Name"].localeCompare(currentProjects[1]["Name"])
          );
